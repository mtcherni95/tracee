...
func NewEvent(ctx tracee.Context, argMetas []external.ArgMeta, args []interface{}, StackAddresses []uint64) (external.Event, error) {
	e := external.Event{
		...
		EventName:           tracee.EventsIDToEvent[int32(ctx.EventID)].Name,
...
func (p *tableEventPrinter) Stream(event *external.Event) {
	if p.verbose {
		if p.containerMode {
...
func (p *tableEventPrinter) Epilogue(stats tracee.StatsStore) {
	fmt.Println()
...
func (p *jsonEventPrinter) Stream(event *external.Event) {
...
func (p *jsonEventPrinter) Epilogue(tracee.StatsStore) {}
...
func (p *gobEventPrinter) Stream(event *external.Event) {
...
func (p *gobEventPrinter) Epilogue(tracee.StatsStore) {}
...
func (p *templateEventPrinter) Stream(event *external.Event) {
...
func (p *templateEventPrinter) Epilogue(store tracee.StatsStore) {}
